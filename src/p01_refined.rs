...
impl<R: SecretRotation + Default + Clone + Send + 'static> P01Topic<R> {
    pub async fn bootstrap(
        ...
        initial_secret_hash: [u8; 32],
        secret_rotation_function: Option<R>,
    ) -> Result<(P01GossipSender, P01GossipReceiver)> {
        ...
        loop {
            ...
            let records = P01Topic::get_unix_minute_records(
                &topic_id.clone(),
                unix_minute,
                secret_rotation_function.clone(),
                initial_secret_hash,
                ...
                if unix_minute != last_published_unix_minute {
                    ...
                    .is_ok()
                    {
                        ...
                        println!(
                            ...
                            z32::encode(
                                P01Topic::<R>::signing_keypair(&topic_id, unix_minute)
                                    .verifying_key()
                                    .as_bytes()
                            )
                        );
    ...
    async fn publish_proc(
        ...
        let records = P01Topic::<R>::get_unix_minute_records(
            &topic_id.clone(),
            unix_minute,
            secret_rotation_function.clone(),
            initial_secret_hash,
        ...
        P01Topic::<R>::publish_unix_minute_record(
            unix_minute,
            &topic_id.clone(),
            secret_rotation_function.clone(),
            initial_secret_hash,
...
impl<R: SecretRotation + Default + Clone + Send + 'static> P01Topic<R> {
    ...
    fn salt(topic_id: &P01TopicId, unix_minute: u64) -> [u8; 32] {
    ...
    async fn get_unix_minute_records(
        ...
        let salt = P01Topic::<R>::salt(&topic_id, unix_minute);
